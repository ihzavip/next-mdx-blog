    ...
    <section className="py-20">
      <div className="container mx-auto px-4 md:px-10">
        ...
        <div className="grid grid-cols-12 gap-8">
          {postsMeta?.map((meta) => (
            <div key={meta.slug} className="col-span-12 md:col-span-4 p-2 flex flex-col">
              ...
              <div className="flex flex-col space-y-2 p-2">
                <Link href={`/posts/${meta.slug}`}>
                  <a className="headingxs cursor-pointer hover:underline">{meta.title}</a>
                </Link>

                <p className="text-sm">{meta.date.split(' ').slice(0, 4).join(' ')}</p>
              </div>
            </div>
          ))}
        </div>
